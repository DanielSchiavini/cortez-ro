...
// ----- STATS CALCULATION -----
	...
	for(i=0;i<(int)sd->status.job_level && i<MAX_LEVEL;i++){
		...
		switch(job_bonus[index][i]) {
			case 1: status->str+=2; break;
			case 2: status->agi+=2; break;
			case 3: status->vit+=2; break;
			case 4: status->int_+=2; break;
			case 5: status->dex+=2; break;
			case 6: status->luk+=2; break;
		}
	}
...
// ----- FLEE CALCULATION -----
	...
	if((skill=pc_checkskill(sd,TF_MISS))>0)
		status->flee += skill*(sd->class_&JOBL_2 && (sd->class_&MAPID_BASEMASK) == MAPID_THIEF? 12 : 10);
...
int status_change_start(struct block_list* bl,enum sc_type type,int rate,int val1,int val2,int val3,int val4,int tick,int flag)
{
	...
	case SC_DONTFORGETME:
		...
		status_change_end(bl,SC_CARTBOOST,-1); // cortez
		break;
	case SC_ONEHAND:
	...
	case SC_BERSERK:
		...
		if(sd)
			strcpy(sd->away_message, "Jogador em frenesi");
		break;
	case SC_ASSUMPTIO:
	...
	switch(type)
	{
		...
		case SC_EDP:	// [Celest]
			...
			val3 = 10*(val1+1); //Damage increase (+50 +50*lv%)
			break;
		case SC_POISONREACT:
		...
		case SC_DEFENDER:
			...
			{	
				...
				val3 = 0; // unused, previously speed adjustment
				val4 = 250 - 70*val1; //Aspd adjustment 
