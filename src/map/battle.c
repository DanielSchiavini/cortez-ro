...
static struct Damage battle_calc_weapon_attack(struct block_list *src,struct block_list *target,int skill_num,int skill_lv,int wflag)
{
	...
	if(skill_num){
		...
		switch(skill_num)
		{
			...
			case KN_PIERCE:
			case ML_PIERCE:
				wd.div_= (wd.div_>0?tstatus->size+2:-(tstatus->size+2));
				break;
			...
			switch( skill_num )
			{
				...
				case MO_EXTREMITYFIST:
					{	//Overflow check. [Skotlex]
						unsigned int ratio = skillratio + 100*(4 + sstatus->sp/20);
						//You'd need something like 6K SP to reach this max, so should be fine for most purposes.
						if (ratio > 25000) ratio = 25000; //We leave some room here in case skillratio gets further increased.
...
struct Damage battle_calc_misc_attack(struct block_list *src,struct block_list *target,int skill_num,int skill_lv,int mflag)
{
	...
	case CR_ACIDDEMONSTRATION: // updated the formula based on a Japanese formula found to be exact [Reddozen]
		if(tstatus->vit+sstatus->int_) //crash fix
			md.damage = (int64)5*tstatus->vit*sstatus->int_*sstatus->int_ / (10*(tstatus->vit*3+sstatus->int_));
		else
			md.damage = 0;
		if (tsd) md.damage>>=1;
...
int battle_check_target( struct block_list *src, struct block_list *target,int flag)
{
	...
	{ //Check rivalry settings.
		...
		if( flag&(BCT_PARTY|BCT_ENEMY) )
		{
			...
			if( s_party && s_party == status_get_party_id(t_bl) && !(map[m].flag.pvp && map[m].flag.pvp_noparty) && !(map_flag_gvg(m) && map[m].flag.gvg_noparty) && !(s_guild && t_guild && s_guild == t_guild) && (!map[m].flag.battleground || sbg_id == tbg_id) )
				state |= BCT_PARTY;
			else
				state |= BCT_ENEMY;
		}
...
// BattleGround Settings
	...
	{ "allow_es_magic_player2",             &battle_config.allow_es_magic_pc2,              0,      0,      1,              },
};
