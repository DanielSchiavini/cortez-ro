...
int atcommand_party(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", party) < 1) {
		clif_displaymessage(fd, "Por favor, digite o nome para o grupo (@party <nome>).");
		return -1;
	}
...
int atcommand_guild(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", guild) < 1) {
		clif_displaymessage(fd, "Por favor, digite o nome do clã (@guild <nome>).");
		return -1;
	}
...
int atcommand_exp(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	sprintf(output, "Base: %d (%.3f%%) | Classe: %d (%.3f%%)", sd->status.base_level, nextb, sd->status.job_level, nextj);
...
int atcommand_changegm(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	{
		clif_displaymessage(fd, "Você precisa ser o líder do clã para usar este comando.");
		return -1;
	}

	if( map[sd->bl.m].flag.guildlock )
	{
		clif_displaymessage(fd, "Você não pode passar o clã para outro jogador enquanto estiver neste mapa.");
		return -1;
	}

	if (strlen(message)==0)
	{
		clif_displaymessage(fd, "Uso do comando: @changegm <nome do novo líder>");
		return -1;
	}
	
	if((pl_sd=map_nick2sd((char *) message)) == NULL || pl_sd->status.guild_id != sd->status.guild_id) {
		clif_displaymessage(fd, "O novo líder precisa estar online e ser membro do clã.");
		return -1;
	}
...
int atcommand_changeleader(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (strlen(message)==0)
	{
		clif_displaymessage(fd, "Uso do comando: @changeleader <nome do jogador já no grupo>");
		return -1;
	}
...
int atcommand_autoloot(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
    ...
    if(!pc_can_give_items(pc_isGM(sd))) rate = 10000;
    
	sd->state.autoloot = rate;
	if (sd->state.autoloot) {
		snprintf(atcmd_output, sizeof atcmd_output, "Pegando itens com chance de %0.02f%% ou menos automaticamente.",((double)sd->state.autoloot)/100.);
		clif_displaymessage(fd, atcmd_output);
	}else
		clif_displaymessage(fd, "Autoloot desligado.");
...
int atcommand_mobsearch(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%99[^\n]", mob_name) < 1) {
		clif_displaymessage(fd, "Por favor, digite o nome do monstro (@mobsearch <nome>).");
		return -1;
	}
	...
	if(mob_id > 0 && mobdb_checkid(mob_id) == 0){
		snprintf(atcmd_output, sizeof atcmd_output, "O ID %s é inválido!",mob_name);
		clif_displaymessage(fd, atcmd_output);
		return -1;
	}
...
//				strcpy(mob_name,mob_db(mob_id)->name);	// --en--
	...
	snprintf(atcmd_output, sizeof atcmd_output, "Procurando monstro %s em %s...", mob_name, mapindex_id2name(sd->mapindex));
	clif_displaymessage(fd, atcmd_output);
...
int atcommand_identify(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (num > 0) {
		...
		clif_displaymessage(fd,"Você não tem itens para identificar.");
	}
	return 0;
}
...
int atcommand_mobinfo(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message) {
		clif_displaymessage(fd, "Por favor, digite um monstro/ID (@mobinfo <nome_ou_id_do_monstro>).");
		return -1;
	}
	...
	for (k = 0; k < count; k++) {
		...
		clif_displaymessage(fd, atcmd_output);
		sprintf(atcmd_output, " Nível:%d  HP:%d  SP:%d  EXP base:%u  EXP classe:%u", mob->lv, mob->status.max_hp, mob->status.max_sp, mob->base_exp, mob->job_exp);
		clif_displaymessage(fd, atcmd_output);
		sprintf(atcmd_output, " DEF:%d  DEF.M:%d  FOR:%d  AGI:%d  VIT:%d  INT:%d  DES:%d  SOR:%d",
			mob->status.def, mob->status.mdef, mob->status.str, mob->status.agi,
			mob->status.vit, mob->status.int_, mob->status.dex, mob->status.luk);
		clif_displaymessage(fd, atcmd_output);
		
		sprintf(atcmd_output, " ATQ:%d~%d  Distância:%d~%d~%d  Tamanho:%s  Raça: %s  Elemento: %s (Lv:%d)",
		...
		if (j == 0)
			clif_displaymessage(fd, "Este monstro não deixa cair nenhum item.");
		else if (j % 3 != 0)
			clif_displaymessage(fd, atcmd_output);
		// mvp
		if (mob->mexp) {
			sprintf(atcmd_output, " EXP MVP:%u  %02.02f%%", mob->mexp, (float)mob->mexpper / 100);
			clif_displaymessage(fd, atcmd_output);
			strcpy(atcmd_output, " Itens MVP:");
			j = 0;
			for (i = 0; i < 3; i++) {
			...
			if (j == 0)
				clif_displaymessage(fd, "Este monstro não dá premios ao MVP.");
			else
				clif_displaymessage(fd, atcmd_output);
		}
	}
...
int atcommand_showmobs(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if((mob_id = atoi(mob_name)) == 0)
		mob_id = mobdb_searchname(mob_name);
	if(mob_id > 0 && mobdb_checkid(mob_id) == 0){
		snprintf(atcmd_output, sizeof atcmd_output, "Monstro %s é inválido!",mob_name);
		clif_displaymessage(fd, atcmd_output);
		return 0;
	}
...
//#define SHOW_MVP
	...
	if(mob_db(mob_id)->status.mode&MD_BOSS){
		snprintf(atcmd_output, sizeof atcmd_output, "Não é possível localizar MVPs!");
		clif_displaymessage(fd, atcmd_output);
		return 0;
	}
#endif
	...
	snprintf(atcmd_output, sizeof atcmd_output, "Procurando monstro %s em %s...",
...
{
	char *itype[12] = {"Poção/Comida", "BUG!", "Usável", "Etc", "Arma", "Armadura", "Carta", "Ovo", "Pet Acessory", "BUG!", "Flecha"};
	...
	if (!message || !*message) {
		clif_displaymessage(fd, "Por favor, digite o nome ou ID do item (@ii/@iteminfo <Nome/ID>).");
		return -1;
	}
	...
	for (i = 0; i < count; i++) {
		...
		sprintf(atcmd_output, "Item: '%s'/'%s'[%d] (%d) Tipo: %s | Efeito: %s",
			item_data->name,item_data->jname,item_data->slot,item_data->nameid,
			item_data->type < 12 ? itype[item_data->type] : "BUG!", 
			(item_data->script==NULL)? "Não" : "Sim"
		);
		clif_displaymessage(fd, atcmd_output);

		sprintf(atcmd_output, "Compra:%dz%s, Venda:%dz%s | Peso: %.1f ", item_data->value_buy, item_data->flag.value_notdc ? "(Sem desconto!)":"", item_data->value_sell, item_data->flag.value_notoc ? "(Sem superfaturar!)":"", item_data->weight/10. );
		clif_displaymessage(fd, atcmd_output);

		if (item_data->maxchance == 10000)
			strcpy(atcmd_output, " - Só disponível em NPCs");
		else if (item_data->maxchance)
			sprintf(atcmd_output, " - Chance máxima em monstros: %02.02f%%", (float)item_data->maxchance / 100 );
		else
			strcpy(atcmd_output, " - Nenhum monstro deixa cair este item");
		clif_displaymessage(fd, atcmd_output);
...
int atcommand_shopsearch(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%99[^\n]", item_name) < 1) {
		clif_displaymessage(fd, "Por favor, digite o número ou nome de um item  (uso: @whosells <Nome ou ID do item>).");
		return -1;
	}
	if ((item_data = itemdb_searchname(item_name)) == NULL &&
	    (item_data = itemdb_exists(atoi(item_name))) == NULL)
		...
		if( pl_sd->vender_id ) //check if player is vending
		{
			for (j = 0; j < pl_sd->vend_num; j++) {
				if(pl_sd->status.cart[pl_sd->vending[j].index].nameid == item_id) {
					...
					sprintf(atcmd_output, "%s (%s %d %d) está vendendo %d.  Preço: %d", pl_sd->status.name, mapindex_id2name(pl_sd->mapindex), pl_sd->bl.x, pl_sd->bl.y, pl_sd->vending[j].amount, pl_sd->vending[j].value);
	...
	if(count > 1) {
		sprintf(atcmd_output, "Localizados %d sendo vendidos. Preços: de %d a %d.", count, MinPrice, MaxPrice);
		clif_displaymessage(fd, atcmd_output);
	} else if(count == 0)
		clif_displaymessage(fd, "Item não localizado.");
...
int atcommand_rates(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	sprintf(buf, "Rates de XP: Base %.1fx / Classe %.1fx",
...
int atcommand_itemlist(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	for( i = 0; i < size; ++i )
	{
		...
		if( count == 1 )
		{
			StringBuf_Printf(&buf, "------ %s do jogador '%s' ------", location, sd->status.name);
		...
		{// pet egg
			if (it->card[3])
				StringBuf_Printf(&buf, " -> (ovo do pet: %u, com nome)", (unsigned int)MakeDWord(it->card[1], it->card[2]));
			else
				StringBuf_Printf(&buf, " -> (ovo do pet id: %u, sem nome)", (unsigned int)MakeDWord(it->card[1], it->card[2]));
		}
		else
		if(it->card[0] == CARD0_FORGE)
		{// forged item
			StringBuf_Printf(&buf, " -> (item forjado, id do criador: %u, fortificado %dx, elemento %d)", (unsigned int)MakeDWord(it->card[2], it->card[3]), it->card[1]>>8, it->card[1]&0x0f);
		}
		else
	...
	if( count == 0 )
		StringBuf_Printf(&buf, "Nenhum item achado no %s do jogador.", location);
	else
		StringBuf_Printf(&buf, "%d item(s) achado(s) em %d slots de %s.", counter, count, location);
...
AtCommandInfo atcommand_info[] = {
	...
	{ "whosell",		    1,1,      atcommand_shopsearch },
	{ "whosells",		    1,1,      atcommand_shopsearch },
